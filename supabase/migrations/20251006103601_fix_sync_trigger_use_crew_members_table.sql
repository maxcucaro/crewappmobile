/*
  # Fix sync trigger - Use correct table name

  ## Problem
  - The trigger `sync_warehouse_checkout_to_completed_shifts` references `crew_profiles` which doesn't exist
  - This causes checkout to fail with error: "relation crew_profiles does not exist"

  ## Solution
  - Replace `crew_profiles` with `crew_members` (the correct table name)
  - Use `first_name` and `last_name` instead of `nome` and `cognome`

  ## Changes
  - Update the function to query `crew_members` instead of `crew_profiles`
  - Update column names to match the actual schema
*/
