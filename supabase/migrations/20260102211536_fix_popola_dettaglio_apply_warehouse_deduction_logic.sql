/*
  # Fix popola_dettaglio_giorno_semplice - Apply warehouse hours deduction
  
  1. Changes
    - Read ha_turno_magazzino from extra_shifts_checkins
    - Store it in extra_ha_turno_magazzino field
    - Calculate extra_benefit correctly:
      - If ha_turno_magazzino = true: (effective_minutes - 480) / 60 * hourly_rate
      - If ha_turno_magazzino = false: effective_minutes / 60 * hourly_rate
    
  2. Why
    - When an employee works both warehouse and extra shift, the first 8 hours
      are already covered by base salary
    - Only hours beyond 8 should be compensated as extra benefits
*/
...
v_extra_ha_magazzino boolean := false;
...
v_assenza_note text;
v_assenza_ora_inizio time;
v_assenza_ora_fine time;
v_benefit_giornaliero numeric := 0;
BEGIN
-- Check ferie/permessi
SELECT 
  tipo_richiesta, 
  COALESCE(note, motivo),
  orario_inizio,
  orario_fine
INTO v_assenza_tipo, v_assenza_note, v_assenza_ora_inizio, v_assenza_ora_fine
...
IF v_assenza_tipo IS NOT NULL THEN
  -- Map tipo_richiesta to correct tipo_giornata
  IF v_assenza_tipo = 'ferie' THEN
    IF v_assenza_ora_inizio IS NOT NULL AND v_assenza_ora_fine IS NOT NULL THEN
      v_tipo_giornata := 'ferie_ore';
    ELSE
      v_tipo_giornata := 'ferie_giornata_intera';
    END IF;
  ELSIF v_assenza_tipo = 'permesso' THEN
    IF v_assenza_ora_inizio IS NOT NULL AND v_assenza_ora_fine IS NOT NULL THEN
...
v_tipo_giornata := 'magazzino';
END IF;

-- Eventi - USA TIMESHEET_ENTRIES
SELECT
e.title,
te.start_time::time,
te.end_time::time,
...
v_tipo_giornata := 'evento';
v_benefit_giornaliero := v_benefit_giornaliero + COALESCE(v_evento_total, 0);
END IF;

-- Turni extra - APPLICA LOGICA SOTTRAZIONE 8 ORE SE HA MAGAZZINO
SELECT
esc.check_in_time::time,
esc.check_out_time::time,
esc.effective_ore_lavorate_minuti,
COALESCE(esc.ha_turno_magazzino, false),
esc.benefit_importo_orario,
-- Calcola benefit corretto: se ha magazzino sottrai 480 minuti (8 ore)
CASE 
  WHEN COALESCE(esc.ha_turno_magazzino, false) THEN 
    ROUND((GREATEST(COALESCE(esc.effective_ore_lavorate_minuti, 0) - 480, 0) / 60.0) * COALESCE(esc.benefit_importo_orario, 0), 2)
  ELSE 
    ROUND((COALESCE(esc.effective_ore_lavorate_minuti, 0) / 60.0) * COALESCE(esc.benefit_importo_orario, 0), 2)
END,
COALESCE(esc."NoteTurno", esc.notes)
INTO v_extra_checkin, v_extra_checkout, v_extra_minuti, v_extra_ha_magazzino, v_extra_tariffa, v_extra_benefit, v_extra_note
...
v_extra_checkin, v_extra_checkout, v_extra_minuti, v_extra_ha_magazzino, v_extra_tariffa, v_extra_benefit, v_extra_note,
...
extra_ha_turno_magazzino = EXCLUDED.extra_ha_turno_magazzino,
