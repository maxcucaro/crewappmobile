/*
  # Fix sync trigger - Convert time to timestamp

  ## Problem
  - check_in_turno and check_out_turno are timestamp with time zone
  - NEW.check_in_time and NEW.check_out_time are time without time zone
  - PostgreSQL cannot implicitly convert between these types

  ## Solution
  - Combine NEW.date (date) with NEW.check_in_time/check_out_time (time)
  - Use the || operator or CAST to create a proper timestamp

  ## Changes
  - Convert time values to timestamps by combining with date
*/
...
BEGIN
  ...
  IF NEW.check_out_time IS NOT NULL 
    ...
    -- Converti time to timestamp combinando date + time
    INSERT INTO crew_turni_completati (
      ...
      NEW.date,
      (NEW.date || ' ' || NEW.check_in_time)::timestamptz,
      (NEW.date || ' ' || NEW.check_out_time)::timestamptz,
