/*
  # Fix popola_dettaglio_giorno to use effective_ore_lavorate_minuti for extra shifts
  
  1. Changes
    - Use effective_ore_lavorate_minuti instead of rectified_total_minutes/net_minutes
    - This ensures benefit calculation uses the same hours as the frontend
    
  2. Why
    - effective_ore_lavorate_minuti is the computed field that considers:
      - Rectified times if present
      - Original times otherwise
      - All breaks (pranzo + cena)
    - This is what we use in the frontend to calculate benefits
*/
...
-- Turni extra - USA EFFECTIVE_ORE_LAVORATE_MINUTI
SELECT
