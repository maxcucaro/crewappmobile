...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'enable_update_requests' AND polrelid = 'registration_requests'::regclass
  ) THEN
    -- Solo gli utenti autenticati con email contenente "admin" possono aggiornare le richieste
    EXECUTE 'CREATE POLICY "enable_update_requests" ON registration_requests
      FOR UPDATE TO authenticated
      USING ((auth.jwt() ->> ''email''::text) ~~ ''%admin%''::text)
      WITH CHECK ((auth.jwt() ->> ''email''::text) ~~ ''%admin%''::text)';
  END IF;
END
$$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'enable_admin_update' AND polrelid = 'registration_requests'::regclass
  ) THEN
    -- Gli admin possono aggiornare tutte le richieste
    EXECUTE 'CREATE POLICY "enable_admin_update" ON registration_requests
      FOR UPDATE TO authenticated
      USING (true)';
  END IF;
END
