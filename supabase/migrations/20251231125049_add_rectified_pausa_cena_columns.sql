/*
  # Aggiungi colonne rettificate per pausa cena

  1. Modifiche
    - Aggiunge `rectified_pausa_cena_inizio` (time) a warehouse_checkins
    - Aggiunge `rectified_pausa_cena_fine` (time) a warehouse_checkins
    - Aggiunge `rectified_pausa_cena_inizio` (time) a extra_shifts_checkins
    - Aggiunge `rectified_pausa_cena_fine` (time) a extra_shifts_checkins
    - Aggiorna la view warehouse_checkins_enriched

  2. Motivo
    - Permette di rettificare gli orari della pausa cena nei turni extra
    - Mantiene coerenza con le altre colonne di rettifica
*/

-- Aggiungi colonne a warehouse_checkins
ALTER TABLE warehouse_checkins 
ADD COLUMN IF NOT EXISTS rectified_pausa_cena_inizio time,
ADD COLUMN IF NOT EXISTS rectified_pausa_cena_fine time;
...
-- Commenti
COMMENT ON COLUMN warehouse_checkins.rectified_pausa_cena_inizio IS 'Orario inizio pausa cena rettificato';
COMMENT ON COLUMN warehouse_checkins.rectified_pausa_cena_fine IS 'Orario fine pausa cena rettificato';
COMMENT ON COLUMN extra_shifts_checkins.rectified_pausa_cena_inizio IS 'Orario inizio pausa cena rettificato';
COMMENT ON COLUMN extra_shifts_checkins.rectified_pausa_cena_fine IS 'Orario fine pausa cena rettificato';
