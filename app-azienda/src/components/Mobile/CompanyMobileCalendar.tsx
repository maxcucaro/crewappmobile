...
const CompanyMobileCalendar: React.FC = () => {
  ...
  const loadAbsences = async () => {
    ...
    try {
      ...
      const absenceRecords:  AbsenceRecord[] = (requests || []).map(req => ({
        id: req.id,
        crewId: req. dipendente_id,
        crewName: req.dipendente_nome || 'N/D',
        type: req.tipo_richiesta,
        startDate: req.data_inizio,
        endDate: req. data_fine,
        reason:  req.motivo
      }));
  ...
  const renderAgendaView = () => {
    ...
    // Combina date con eventi e assenze
                ...
                <div className={`p-4 border-b border-gray-700 ${isToday ? 'bg-blue-600' : 'bg-gray-750'}`}>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-sm text-gray-300 font-medium">
                      {dayEvents.length > 0 && `${dayEvents.length} ${dayEvents.length === 1 ? 'evento' : 'eventi'}`}
                      {dayEvents.length > 0 && dayAbsences.length > 0 && ' • '}
                      {dayAbsences.length > 0 && `${dayAbsences.length} ${dayAbsences.length === 1 ? 'assenza' : 'assenze'}`}
                    </div>
                  </div>
                </div>
                  ...
                  {dayAbsences.length > 0 && (
                    <div className="bg-red-900/20 border border-red-700 rounded-lg p-3">
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        <h4 className="text-sm font-bold text-red-300">Dipendenti Assenti</h4>
                      </div>
                        ...
                        {dayAbsences.map(absence => {
                          const getAbsenceTypeLabel = (type: string) => {
                            switch (type) {
                              ...
                              case 'infortunio': return 'Infortunio';
                              default: return type;
                            }
                          };
                          ...
                          return (
                            <div key={absence.id} className="flex items-center justify-between text-sm">
                              <span className="text-white">{absence.crewName}</span>
                    ...
                    >
                      <div className="flex items-start justify-between mb-1">
                        ...
                        {day.absences.length > 0 && (
                          <div
                            className="h-2 w-2 rounded-full bg-red-500 shadow-lg mt-0.5"
                            title={`${day.absences.length} assenze approvate`}
                          />
                        )}
                      </div>
