...
import { Clock, RefreshCw, Check, X, User, AlertCircle, Calendar } from 'lucide-react';
...
const CompanyStraordinari: React.FC = () => {
  ...
  const loadRequests = async () => {
    ...
    try {
      let query = supabase
        ...
        .select(`
          *,
          crew_member:crew_members!crew_richieste_straordinari_dipendente_id_fkey(
            nome,
            cognome,
            email
          )
  ...
  const handleApprove = async (requestId: string) => {
    ...
    try {
      ...
      showSuccess('Straordinario approvato');
      await loadRequests();
    } catch (err: any) {
  ...
  const handleReject = async (requestId: string) => {
    ...
    try {
      ...
      showSuccess('Straordinario rifiutato');
  ...
  const totalPendingHours = requests
    ...
    .reduce((sum, r) => sum + (r.ore_straordinarie || 0), 0);
    ...
    <div className="space-y-4">
      <div className="bg-gray-800 rounded-xl p-4 border border-gray-700">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xl font-bold text-white">Ore Straordinarie</h2>
        ...
        {pendingCount > 0 && (
          <div className="mb-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
            ...
            <div className="flex items-center gap-2 text-lg font-bold text-yellow-400 ml-8">
              ...
              {totalPendingHours.toFixed(2)} ore totali
            </div>
          </div>
        )}
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          ...
          >
            Approvati ({requests.filter(r => r.stato === 'approvata').length})
          </button>
          <button
          ...
          >
            Rifiutati ({requests.filter(r => r.stato === 'rifiutata').length})
          </button>
          <button
              ...
              <div key={request.id} className="bg-gray-800 rounded-xl p-4 border border-gray-700">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="text-right">
                    <div className="text-xl font-bold text-white mb-1">
                      {request.ore_straordinarie.toFixed(2)}h
                    </div>
                ...
                <div className="space-y-2 mb-3">
                  <div className="flex items-center gap-2 text-sm text-gray-300">
                    ...
                    <span>{formatDate(request.data_straordinario)}</span>
                  </div>
                  ...
                  {request.approved_by && (
                    <div className="text-xs text-gray-500 mt-2">
                      Gestito da: {request.approved_by} • {formatDate(request.approved_at)}
                    </div>
                  )}
                </div>
      ...
      {showRejectModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 rounded-xl p-6 max-w-md w-full border border-gray-700">
            <h3 className="text-xl font-bold text-white mb-4">Rifiuta Straordinario</h3>
            ...
            <textarea
              ...
              rows={4}
              placeholder="Es: Ore non autorizzate, turno non previsto..."
            />
...
export default CompanyStraordinari;
