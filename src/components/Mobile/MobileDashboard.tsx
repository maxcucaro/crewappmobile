...
const MobileDashboard: React.FC<MobileDashboardProps> = ({ onNavigate }) => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      console.log('🔍 Caricamento dashboard con sistema benefit per user:', user?.id);
      ...
      console.log('📅 Periodo analisi:', { startOfMonth, endOfMonth, todayString });
      ...
      if (turniError) {
        console.error('❌ Errore caricamento turni:', turniError);
      }

      console.log('🏭 Turni magazzino del mese corrente:', turniData?.length || 0);
      ...
      if (assignmentsError) {
        console.error('❌ Errore caricamento assegnazioni eventi:', assignmentsError);
        setEventsWithBenefits([]);
        return;
      }

      console.log('📅 Eventi assegnati trovati:', assignmentsData?.length || 0);
      ...
      if (warehouseShiftsError) {
        console.error('❌ Errore caricamento turni magazzino:', warehouseShiftsError);
        setWarehouseShifts([]);
      } else {
        console.log('🏭 Turni magazzino assegnati trovati:', warehouseShiftsData?.length || 0);
      ...
      if (ratesError) {
        console.error('❌ Errore caricamento tariffe dipendente:', ratesError);
        return;
      }

      console.log('💰 Tariffe dipendente caricate:', employeeRatesData?.length || 0);
      ...
      console.log('🎁 Benefit dipendente disponibili:', employeeBenefits.map(b => b.nome_tariffa));

      // 6. Per ogni evento, calcola i benefit applicabili
      const eventsWithCalculatedBenefits: EventWithBenefits[] = [];

      for (const assignment of assignmentsData || []) {
        console.log(`\n🎭 ANALISI EVENTO: "${assignment.nome_evento}"`);
        console.log('📋 Benefit evento disponibili:', {
        ...
        if (assignment.benefits_evento_nomi && assignment.benefits_evento_nomi.length > 0) {
          assignment.benefits_evento_nomi.forEach((benefitName: string) => {
            ...
            if (matchingBenefit) {
              ...
              console.log(`❌ BENEFIT NON DISPONIBILE: ${benefitName} (dipendente non ha questa tariffa)`);
            }
          });
        }
        ...
        if (assignment.bonus_trasferta && assignment.evento_trasferta) {
          ...
          if (trasfertaBenefit && !applicableBenefits.find(b => b.id === trasfertaBenefit.id)) {
            ...
            console.log(`✅ BONUS TRASFERTA APPLICATO: ${trasfertaBenefit.nome_tariffa} = €${trasfertaBenefit.importo}`);
          }
        }
        ...
        if (assignment.bonus_diaria) {
          ...
          if (diariaBenefit && !applicableBenefits.find(b => b.id === diariaBenefit.id)) {
            ...
            console.log(`✅ BONUS DIARIA APPLICATO: ${diariaBenefit.nome_tariffa} = €${diariaBenefit.importo}`);
          }
        }
        ...
        console.log(`💰 TOTALE BENEFIT APPLICABILI: €${totalBenefitsAmount}`);
        ...
        console.log(`📊 DETTAGLIO BENEFIT:`, benefitDetails);
  ...
  const calculateStatsFromDatabase = async (events: EventWithBenefits[], turniMagazzino: any[], warehouseShifts: any[]) => {
    ...
    // I turni sono già filtrati per il mese corrente dalla query
    ...
    try {
      ...
      console.error('❌ Errore caricamento dati dal database:', error);
    }
    ...
    console.log('📊 DEBUG EVENTI TRASFERTA:', {
      monthlyEventsCount: monthlyEvents.length,
      eventiConTrasferta: monthlyEvents.filter(e => e.assignment.evento_trasferta).map(e => ({
        nome: e.assignment.nome_evento,
        evento_trasferta: e.assignment.evento_trasferta
      })),
      travelEventsCount: travelEvents
    });
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      <div className="p-4 pb-20 space-y-6">
        ...
        <StatsGrid monthlyStats={monthlyStats} onNavigate={onNavigate} />
