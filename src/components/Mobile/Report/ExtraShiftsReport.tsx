...
import { Star, Clock, Coffee, Utensils, Gift, FileText, AlertTriangle, CheckCircle, Filter, Euro } from 'lucide-react';
import ShiftActions from './ShiftActions';
import { toItalianTime } from '../../../utils/dateUtils';
import { formatMinutesAsTime, getExtraShiftBenefitDetails } from '../../../utils/timeFormat';
...
interface CompletedShift {
  ...
  benefit_tariffa_id?: string;
  benefit_importo_orario?: number;
  [key: string]: any;
}

interface ExtraShiftsReportProps {
  selectedMonth: number;
  selectedYear: number;
}

const ExtraShiftsReport: React.FC<ExtraShiftsReportProps> = ({ selectedMonth, selectedYear }) => {
  const { user } = useAuth();
  const [shifts, setShifts] = useState<CompletedShift[]>([]);
  const [filteredShifts, setFilteredShifts] = useState<CompletedShift[]>([]);
  const [selectedDate, setSelectedDate] = useState<string>('');
  const [selectedShiftName, setSelectedShiftName] = useState<string>('all');
  const [availableShiftNames, setAvailableShiftNames] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    if (selectedShiftName !== 'all') {
      filtered = filtered.filter(shift => {
        const locationData = shift.notes || '';
        return locationData.includes(selectedShiftName);
      });
    }

    setFilteredShifts(filtered);
  }, [shifts, selectedDate, selectedShiftName]);

  const loadExtraShiftsData = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      const { data: extraShiftsData, error: extraShiftsError } = await supabase
      ...
      if (extraShiftsError) throw extraShiftsError;

      const mappedExtraShifts = (extraShiftsData || []).map((shift: any) => {
        ...
        const pausaPranzo = shift.rectified_pausa_pranzo !== null ? shift.rectified_pausa_pranzo : (shift.has_taken_break || false);
        ...
        return {
          ...
          nome_turno: 'TURNO EXTRA',
          check_in_turno: checkInTime,
          check_out_turno: checkOutTime,
          conteggio_ore: shift.effective_ore_lavorate_minuti || 0,
          ...
          pausa_totale_minuti: shift.pausa_totale_minuti,
          status: shift.status,
          ...
          original_pausa_pranzo: shift.has_taken_break || false,
          ...
          auto_checkout: false,
          benefit_tariffa_id: shift.benefit_tariffa_id,
          benefit_importo_orario: shift.benefit_importo_orario ? parseFloat(shift.benefit_importo_orario) : undefined
        } as CompletedShift;
      });

      setShifts(mappedExtraShifts);

      const uniqueNames = new Set<string>();
      mappedExtraShifts.forEach((shift: CompletedShift) => {
        if (shift.notes) {
          uniqueNames.add(shift.notes);
        }
      });
      setAvailableShiftNames(Array.from(uniqueNames).sort());
    } catch (err) {
      console.error('Errore caricamento turni extra:', err);
    ...
    return (
      <div className="bg-gray-800 rounded-xl p-4 border border-gray-700">
        <div className="text-center py-8">
          ...
          <p className="text-gray-400">Caricamento turni extra...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-800 rounded-xl p-4 border border-gray-700">
      <h3 className="text-lg font-semibold text-white mb-4">
        Turni Extra ({filteredShifts.length}{shifts.length !== filteredShifts.length ? ` di ${shifts.length}` : ''})
      </h3>
      ...
      <div className="mb-4 space-y-3">
        ...
        {/* Filtro per Nome Turno Extra */}
        {availableShiftNames.length > 0 && (
          <div>
            <label className="block text-xs font-medium text-gray-300 mb-2">
              Filtra per Nome Turno:
            </label>
            <select
            ...
            >
              ...
              {availableShiftNames.map((name, idx) => (
        ...
        {(selectedDate || selectedShiftName !== 'all') && (
          <div className="bg-blue-900 bg-opacity-30 border border-blue-700 rounded-lg p-2 text-xs text-blue-200">
            ...
            <div className="mt-1 space-y-1">
              ...
              {selectedShiftName !== 'all' && (
                <div>Turno: {selectedShiftName}</div>
              )}
            </div>
          </div>
        )}
      ...
      {filteredShifts.length === 0 ? (
        <div className="text-center py-8 text-gray-400">
          ...
          <p>Nessun turno extra {selectedDate ? 'trovato con il filtro selezionato' : 'nel periodo selezionato'}</p>
          <p className="text-sm mt-1">{selectedDate ? 'Prova a modificare il filtro' : 'I turni extra appariranno qui'}</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filteredShifts.map((shift) => {
            ...
            return (
              <div
                key={shift.id}
                className="bg-gradient-to-r from-blue-900 to-blue-800 rounded-lg p-4 border border-blue-600"
              >
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <div className="flex items-center space-x-2">
                      <Star className="h-5 w-5 text-blue-300" />
                      <h4 className="font-bold text-white">TURNO EXTRA</h4>
                      {shift.is_rectified && (
                  ...
                  <div className="text-right">
                    {isInProgress ? (
                      <div className="text-sm text-blue-300 font-semibold">In corso</div>
                    ) : (
                      <>
                        <div className="text-lg font-bold text-blue-300">
                          {shift.effective_ore_lavorate_minuti
                            ? formatMinutesAsTime(shift.effective_ore_lavorate_minuti)
                            : '0h 0min'}
                        </div>
                        <div className="text-xs text-blue-200">Ore Lavorate</div>
                      </>
                    )}
                  </div>
                </div>
                ...
                {/* Orari Check-in/out */}
                <div className="bg-gray-900 bg-opacity-40 rounded-lg p-3 mb-3">
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      <div className="flex items-center space-x-1 mb-1">
                        <Clock className="h-3 w-3 text-green-400" />
                        <span className="text-blue-200 font-medium">Check-in:</span>
                      </div>
                      <div className="text-white font-semibold">
                        {formatTime(shift.check_in_turno) || '--:--'}
                      </div>
                      {shift.is_rectified && shift.original_check_in && (
                        <div className="text-blue-300 line-through text-xs mt-1">
                    ...
                    <div>
                      <div className="flex items-center space-x-1 mb-1">
                        ...
                        <span className="text-blue-200 font-medium">Check-out:</span>
                      </div>
                      {isInProgress ? (
                        <div className="flex flex-col">
                          <div className="text-blue-300 font-semibold">In corso</div>
                ...
                {/* Pause */}
                <div className="bg-gray-900 bg-opacity-40 rounded-lg p-3 mb-3 space-y-2">
                  {/* Pausa Pranzo */}
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <Coffee className="h-4 w-4 text-cyan-400" />
                      <span className="text-sm text-blue-200">Pausa Pranzo:</span>
                    </div>
                    <div className="text-right">
                      {shift.pausa_pranzo ? (
                        <div>
                      ...
                      {shift.is_rectified && shift.original_pausa_pranzo !== shift.pausa_pranzo && (
                        <div className="text-xs text-blue-300 line-through mt-1">
                  ...
                  {(shift.pausa_cena || shift.rectified_pausa_cena_inizio || shift.pausa_cena_inizio) && (
                    <div className="flex items-center justify-between pt-2 border-t border-blue-700">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-blue-200">Pausa Cena:</span>
                      </div>
                      <div className="text-right">
                        {(shift.pausa_cena || shift.rectified_pausa_cena_inizio || shift.pausa_cena_inizio) ? (
                          <div>
                  ...
                  {shift.effective_pausa_totale_minuti > 0 && (
                    <div className="flex items-center justify-between pt-2 border-t border-blue-700">
                      <span className="text-xs text-blue-200">Totale Pause:</span>
                      <span className="text-xs text-white font-semibold">{formatMinutesAsTime(shift.effective_pausa_totale_minuti)}</span>
                    </div>
                  )}
                  ...
                  {shift.effective_ore_lavorate_minuti > 0 && (
                    <div className="flex items-center justify-between pt-2 border-t border-blue-700">
                      <span className="text-xs text-blue-200">Ore Effettive:</span>
                ...
                {/* Benefit Turno Extra */}
                {shift.benefit_tariffa_id && shift.benefit_importo_orario && shift.effective_ore_lavorate_minuti > 0 && hasCheckout && (
                  (() => {
                    const benefitDetails = getExtraShiftBenefitDetails(
                      shift.effective_ore_lavorate_minuti,
                      shift.ore_previste || 480,
                      shift.benefit_importo_orario
                    );

                    if (!benefitDetails.hasBenefit) return null;

                    return (
                      <div className="bg-gradient-to-r from-green-900 to-green-800 rounded-lg p-3 mb-3 border border-green-600">
                        <div className="flex items-start space-x-2">
                          <Euro className="h-5 w-5 text-green-300 flex-shrink-0 mt-0.5" />
                          <div className="flex-1">
                            <div className="text-sm font-bold text-green-200 mb-2">Benefit Turno Extra</div>
                            <div className="space-y-1 text-xs text-green-100">
                              <div className="flex justify-between">
                                <span>Ore Extra Lavorate:</span>
                                <span className="font-semibold">{benefitDetails.actualWorkedFormatted}</span>
                              </div>
                              <div className="flex justify-between">
                                <span>Ore Magazzino Previste:</span>
                                <span className="font-semibold">{benefitDetails.expectedWarehouseFormatted}</span>
                              </div>
                              <div className="flex justify-between pt-1 border-t border-green-700">
                                <span>Ore da Conteggiare:</span>
                                <span className="font-semibold text-green-300">{benefitDetails.extraMinutesFormatted}</span>
                              </div>
                              <div className="flex justify-between">
                                <span>Tariffa Oraria:</span>
                                <span className="font-semibold">€{benefitDetails.hourlyRate.toFixed(2)}/ora</span>
                              </div>
                              <div className="flex justify-between pt-2 border-t border-green-700">
                                <span className="text-base font-bold">Importo Benefit:</span>
                                <span className="text-base font-bold text-green-300">€{benefitDetails.benefit.toFixed(2)}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                ...
                {(shift.noteturno || shift.notes) && (
                  <div className="bg-blue-900 bg-opacity-30 border border-blue-700 rounded-lg p-3 mb-3">
                    <div className="flex items-start space-x-2">
                      <FileText className="h-4 w-4 text-blue-300 flex-shrink-0 mt-0.5" />
                      <div className="flex-1">
                        <div className="text-xs font-semibold text-blue-200 mb-1">Note Dipendente:</div>
                        <div className="text-xs text-blue-100">{shift.noteturno || shift.notes}</div>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="flex flex-wrap gap-2 text-xs mb-2">
                  ...
                  {shift.pasto_aziendale_usufruito && (
                    <span className="bg-blue-600 text-white px-2 py-1 rounded flex items-center space-x-1">
                ...
                {shift.is_rectified && shift.rectification_note && (
                  <div className="bg-blue-900 bg-opacity-50 border border-blue-700 rounded-lg p-3 mb-3">
                    <div className="flex items-start space-x-2">
                      <AlertTriangle className="h-4 w-4 text-blue-300 flex-shrink-0 mt-0.5" />
                      <div>
                        <div className="text-xs font-semibold text-blue-200 mb-1">Nota di Rettifica:</div>
                        <div className="text-xs text-blue-100">{shift.rectification_note}</div>
                      </div>
                    </div>
                  </div>
                )}

                <ShiftActions shift={shift} onUpdate={loadExtraShiftsData} tableName="extra_shifts_checkins" />
              </div>
            );
          })}
        </div>
...
export default ExtraShiftsReport;
