...
const WarehouseCheckIn: React.FC = () => {
  ...
  const [showCheckoutNotePrompt, setShowCheckoutNotePrompt] = useState(false);
  ...
  const checkAndCleanupExpiredSessions = useCallback(async () => {
    ...
    try {
      ...
      // Controlla se il turno è scaduto da più di 4 ore
      if (isTimeAfter(currentTime, endTime)) {
        const hoursLate = calculateHoursLate(endTime, currentTime);

        // Auto-checkout solo se sono passate più di 4 ore dalla fine turno prevista
        if (hoursLate >= 4) {
  ...
  const loadShiftNote = async () => {
    ...
    try {
      ...
      setShiftNote(data.NoteTurno || '');
    } catch (error) {
      console.error('Errore caricamento nota turno:', error);
    }
  };

  const saveShiftNote = async (note: string) => {
    if (!currentSession || currentSession.type !== 'warehouse') {
      showError('Errore', 'Nessuna sessione attiva');
      return;
    }
    ...
    try {
      const { error } = await supabase
        .from('warehouse_checkins')
        .update({ NoteTurno: note.trim() || null })
        .eq('id', currentSession.id);

      if (error) {
        console.error('Errore salvataggio nota turno:', error);
        showError('Errore', 'Impossibile salvare la nota');
        return;
      }

      setShiftNote(note.trim());
      showSuccess('Nota Salvata', 'La nota del turno è stata salvata con successo');
    } catch (error) {
      console.error('Errore salvataggio nota turno:', error);
      showError('Errore', 'Impossibile salvare la nota');
    } finally {
  ...
  // Carica stato pausa e nota quando cambia la sessione
  useEffect(() => {
  ...
  const validateShiftTiming = (shift: WarehouseShift) => {
    ...
    if (isBeforeStart) {
      ...
      // Permetti check-in fino a 4 ore (240 minuti) prima dell'inizio turno
      if (minutesEarly <= 240) {
        return {
          isValid: true,
          reason: `Check-in anticipato: turno inizia alle ${shiftStartTime} (tra ${minutesEarly} minuti)`,
          canCheckIn: true,
          canCheckOut: false,
          isExpired: false,
          isEarlyCheckIn: true
      ...
      // Troppo presto (più di 4 ore prima)
      return {
        isValid: false,
    ...
    try {
      ...
      if (!existingError && existingData) {
        ...
        if (activeCheckIn && (!currentSession || currentSession.id !== activeCheckIn.id)) {
          ...
          // Usa gli orari salvati in warehouse_checkins (se disponibili) o carica dal turno
          let shiftStartTime = activeCheckIn.ora_inizio_turno || '09:00';
          let shiftEndTime = activeCheckIn.ora_fine_turno || '17:00';
          let hasLunchBreak = activeCheckIn.break_minutes > 0;
          let shiftName = 'Turno Magazzino';

          // Se gli orari non sono in warehouse_checkins (check-in vecchi), caricali dal turno
          if (!activeCheckIn.ora_inizio_turno || !activeCheckIn.ora_fine_turno) {
            ...
            shiftStartTime = shiftData?.crew_template_turni?.ora_inizio_turno || shiftData?.ora_inizio_turno || '09:00';
            shiftEndTime = shiftData?.crew_template_turni?.ora_fine_turno || shiftData?.ora_fine_turno || '17:00';
            hasLunchBreak = shiftData?.crew_template_turni?.pausa_pranzo !== false;
  ...
  const handleWarehouseCheckIn = async (warehouse: WarehouseInfo, forceCheckin: boolean = false) => {
    ...
    try {
      ...
      const checkInData = {
        ...
        early_checkin: validation.isEarlyCheckIn || false
      };
      ...
      if (isOnline) {
        ...
        if (error) {
          ...
          // --- FIX: refresh immediato dello stato pausa / checkin in modo che la UI mostri il pulsante pausa senza dover cambiare pagina ---
          try {
  ...
  const handleManualCheckOut = async () => {
    ...
    // Se non c'è una nota salvata, chiedi se vuole aggiungerne una
    if (!shiftNote || shiftNote.trim() === '') {
      setShowCheckoutNotePrompt(true);
    } else {
      // Altrimenti mostra direttamente il modal di conferma checkout
    ...
    <div className="space-y-6">
      ...
      {activeSessions.filter(s => s.type === 'warehouse').length > 0 && (
        <div className="bg-gradient-to-br from-purple-600 to-pink-600 rounded-2xl p-6 shadow-xl">
                    ...
                    <div className="space-y-1">
                      {session.hasCompanyMeal && (
                        <div className="flex justify-between text-sm">
                          <span className="text-purple-200">🍽️ Pasto Aziendale:</span>
          ...
          {currentSession && currentSession.type === 'warehouse' && (
            <div className="bg-white bg-opacity-10 rounded-lg p-4 mt-4">
              <div className="flex items-center justify-between mb-3">
                <h5 className="text-white font-medium flex items-center space-x-2">
                  <span>📝</span>
                  <span>Nota Turno</span>
                </h5>
                {shiftNote && (
                  <span className="text-xs bg-green-500 px-2 py-1 rounded-full text-white">
                    Salvata
                  </span>
                )}
              </div>
              ...
              <div className="space-y-2">
                <textarea
                  ...
                  placeholder="Scrivi qui eventuali note sul turno (opzionale)..."
                  className="w-full bg-white bg-opacity-10 text-white placeholder-gray-400 rounded-lg p-3 border border-white border-opacity-20 focus:border-purple-400 focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none"
                  rows={3}
                />
                <button
                  onClick={() => saveShiftNote(shiftNote)}
                  disabled={savingNote}
                  className="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-gray-600 disabled:cursor-not-allowed text-white py-2 px-4 rounded-lg font-medium transition-colors flex items-center justify-center space-x-2"
                >
                  {savingNote ? (
                    <>
                ...
                <p className="text-xs text-gray-300 text-center">
                  Puoi modificare questa nota in qualsiasi momento durante il turno
                </p>
              </div>
            </div>
          )}
          ...
          >
            {currentSession?.type === 'warehouse' ? '🛑 TERMINA TURNO MAGAZINO' : '🛑 TERMINA EVENTO'}
          </button>
        </div>
      )}
                  ...
                  return (
                    <div key={shift.id} className={`bg-gray-700 rounded-lg p-4 border-l-4 ${hasCheckIn ? 'border-green-500' : 'border-purple-500'}`}>
                      ...
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                        ...
                        {validation.isValid && !validation.isEarlyCheckIn && (
                          <span className="text-xs px-2 py-1 rounded-full bg-green-100 text-green-800">✅ Turno attivo</span>
                        )}
                        {validation.isValid && validation.isEarlyCheckIn && (
                          <span className="text-xs px-2 py-1 rounded-full bg-blue-100 text-blue-800">⏰ Check-in anticipato disponibile</span>
                        )}
                        {hasCheckIn && shiftCheckIn?.early_checkin && (
                          <span className="text-xs px-2 py-1 rounded-full bg-indigo-100 text-indigo-800">🚀 Turno anticipato</span>
                        )}
                      </div>
      ...
      {/* Modal Prompt Nota al Checkout */}
      {showCheckoutNotePrompt && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-xl max-w-md w-full p-6 border border-blue-500">
            <div className="flex items-center space-x-3 mb-4">
              <div className="bg-blue-900 p-3 rounded-full">
                <span className="text-2xl">📝</span>
              </div>
              <h3 className="text-xl font-bold text-white">Nota Turno</h3>
            </div>

            <div className="space-y-4 mb-6">
              <p className="text-gray-300 text-sm">
                Vuoi aggiungere una nota a questo turno prima di terminarlo?
              </p>
              ...
              <textarea
                ...
                onChange={(e) => setShiftNote(e.target.value)}
                placeholder="Scrivi qui eventuali note sul turno (opzionale)..."
                className="w-full bg-gray-700 text-white placeholder-gray-400 rounded-lg p-3 border border-gray-600 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                rows={4}
                autoFocus
              />

              <p className="text-xs text-gray-400">
                La nota è opzionale. Puoi saltare questo passaggio se non hai nulla da aggiungere.
              </p>
            </div>
            ...
            <div className="flex space-x-3">
              <button
                onClick={() => {
                  setShowCheckoutNotePrompt(false);
                  setShowCheckOutConfirmModal(true);
                }}
                className="flex-1 bg-gray-700 text-white py-3 px-4 rounded-lg hover:bg-gray-600 font-medium"
              >
              ...
              <button
                onClick={async () => {
                  if (shiftNote.trim()) {
                    await saveShiftNote(shiftNote);
                  }
                  setShowCheckoutNotePrompt(false);
                  setShowCheckOutConfirmModal(true);
                }}
                disabled={savingNote}
                className="flex-1 bg-blue-600 text-white py-3 px-4 rounded-lg hover:bg-blue-700 font-medium disabled:bg-gray-600 disabled:cursor-not-allowed"
              >
                {savingNote ? 'Salvataggio...' : shiftNote.trim() ? 'Salva e Continua' : 'Continua'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-gray-800 rounded-xl max-w-md w-full p-6 border border-red-500">
            ...
            <div className="space-y-4 mb-6">
              <div className="bg-red-900/20 border border-red-700 rounded-lg p-4">
                ...
                <p className="text-red-200 text-xs">
                  Questa azione registrerà il check-out e chiuderà definitivamente la sessione corrente. Assicurati di aver completato tutte le attività.
                </p>
              </div>
                  ...
                  {currentSession.hasLunchBreak && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-white">
                        {breakInProgress ? '⏸️ In corso' : breakStartTime ? '✅ Completata' : '⚠️ Non registrata'}
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div className="bg-yellow-900/20 border border-yellow-700 rounded-lg p-3">
                <p className="text-yellow-300 text-xs">
                  ⚠️ Dopo il check-out non potrai più modificare questo turno. Verifica che tutti i dati siano corretti.
                </p>
              </div>
            </div>
