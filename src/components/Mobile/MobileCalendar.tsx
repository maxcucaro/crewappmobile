...
const MobileCalendar: React.FC = () => {
  ...
  // Approved vacation days (ferie approvate)
  const [approvedVacationDays, setApprovedVacationDays] = useState<Set<string>>(new Set());
  ...
  const loadUserProfile = async () => {
    try {
      ...
      if (userData?.parent_company_id) {
        ...
        await loadApprovedVacations(personId, userData.parent_company_id);
      } else {
        await loadCompanyEvents(undefined, personId);
        await loadApprovedVacations(personId);
      }
    } catch (err) {
  ...
  const loadApprovedVacations = async (personId?: string | null, companyId?: string) => {
    try {
      if (!personId) return;

      const { data, error } = await supabase
        .from('crew_richiesteferie_permessi')
        .select('data_inizio, data_fine, tipo_richiesta, stato')
        .eq('dipendente_id', personId)
        .eq('stato', 'approvata');

      if (error) {
        console.error('Errore caricamento ferie approvate:', error);
        return;
      }
      ...
      if (data && data.length > 0) {
        data.forEach((vacation: any) => {
          if (vacation.tipo_richiesta === 'ferie') {
            const startDate = new Date(vacation.data_inizio);
            const endDate = new Date(vacation.data_fine);

            let currentDate = new Date(startDate);
            while (currentDate <= endDate) {
              const dateKey = formatDateKey(currentDate);
              vacationDaysSet.add(dateKey);
      ...
      setApprovedVacationDays(vacationDaysSet);
      console.log('✅ Ferie approvate caricate:', vacationDaysSet.size, 'giorni');
    } catch (err) {
      console.error('Eccezione loadApprovedVacations:', err);
    }
  };
  ...
  const getDaysInMonth = (date: Date): CalendarDay[] => {
    ...
    for (let i = 0; i < firstDayOfWeek; i++) {
      ...
      const isVacation = approvedVacationDays.has(formatDateKey(prevDate));
      days.push({ date: prevDate, isCurrentMonth: false, isToday: false, events: getEventsForDate(prevDate), isAvailable: true, isVacationDay: isVacation });
    }
    for (let day = 1; day <= daysInMonth; day++) {
      const currentDay = new Date(year, month, day);
      const dayEvents = getEventsForDate(currentDay);
      const isVacation = approvedVacationDays.has(formatDateKey(currentDay));
      days.push({ date: currentDay, isCurrentMonth: true, isToday: formatDateKey(currentDay) === formatDateKey(new Date()), events: dayEvents, isAvailable: dayEvents.length === 0, isVacationDay: isVacation });
    }
    const remainingCells = 42 - days.length;
    for (let i = 1; i <= remainingCells; i++) {
      const nextDate = new Date(year, month + 1, i);
      const isVacation = approvedVacationDays.has(formatDateKey(nextDate));
      days.push({ date: nextDate, isCurrentMonth: false, isToday: false, events: getEventsForDate(nextDate), isAvailable: true, isVacationDay: isVacation });
    }
    return days;
  };
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-7 gap-3 md:gap-2">
          {weekDays.map(day => {
            ...
            const isVacation = approvedVacationDays.has(dayKey);
            const isToday = formatDateKey(day) === formatDateKey(new Date());

            return (
              <div key={dayKey} className={`rounded-lg p-3 md:p-2 md:min-h-[140px] ${isVacation ? 'bg-red-600' : 'bg-gray-900'}`}>
                <div className={`text-base md:text-sm font-semibold mb-2 ${isToday ? 'text-blue-300' : 'text-white'}`}>
                  {day.toLocaleDateString('it-IT', { weekday: 'long', day: 'numeric', month: 'short' })}
                </div>
                {isVacation ? (
                  <div className="flex items-center justify-center py-4 md:h-20">
                    <span className="text-sm md:text-xs font-bold text-white uppercase tracking-wider">FERIE</span>
                  </div>
                ) : (
                  <div className="space-y-2 md:space-y-1">
                    {dayEvents.map(ev => (
                      <div
                        key={ev.id}
                        className={`text-sm md:text-[11px] leading-tight px-2 md:px-1.5 py-2 md:py-1 rounded text-white font-medium cursor-pointer hover:opacity-90 ${
                          ev.type === 'warehouse'
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <span className="truncate md:block">{ev.title}</span>
                          <span className="text-xs opacity-75 whitespace-nowrap md:hidden">
                            {ev.startTime}
                          </span>
                        </div>
                      </div>
      ...
      <div className="space-y-4">
        {sortedDates.length === 0 ? (
          ...
          sortedDates.map((dateKey) => {
            ...
            const isVacation = approvedVacationDays.has(dateKey);
            const dayEvents = eventsByDate[dateKey];

            return (
              <div key={dateKey} className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
                <div className={`p-4 border-b border-gray-700 ${isVacation ? 'bg-red-600' : isToday ? 'bg-blue-600' : 'bg-gray-750'}`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-lg font-bold text-white">
                        {date.toLocaleDateString('it-IT', { weekday: 'long', day: 'numeric', month: 'long' })}
                      </div>
                      {isVacation && <div className="text-sm text-white font-bold mt-1 uppercase tracking-wider">FERIE</div>}
                      {isToday && !isVacation && <div className="text-sm text-blue-100 mt-1">Oggi</div>}
                    </div>
            ...
            <div className="bg-gray-800 rounded-xl border border-gray-700">
              ...
              <div className="p-2 md:p-3">
                <div className="grid grid-cols-7 gap-1 mb-2">
                  {['Lun','Mar','Mer','Gio','Ven','Sab','Dom'].map(day => <div key={day} className="p-1 md:p-2 text-center text-xs md:text-sm font-semibold text-gray-300">{day}</div>)}
                </div>

                <div className="grid grid-cols-7 gap-1 md:gap-1.5">
                  {getDaysInMonth(currentDate).map((day, index) => {
                    let dayClasses = 'min-h-[70px] md:min-h-[85px] p-1 md:p-1.5 rounded-lg border cursor-pointer transition-colors relative ';

                    if (day.isVacationDay) {
                      dayClasses += 'bg-red-600 text-white border-red-700 shadow-lg ';
                    } else if (!day.isCurrentMonth) {
                      dayClasses += 'bg-gray-800 text-gray-500 border-gray-700 ';
                    } else if (day.isToday) {
                      dayClasses += 'bg-blue-600 text-white border-blue-700 shadow-lg ';
                    } else {
                      dayClasses += 'bg-gray-750 hover:bg-gray-700 border-gray-700 ';
                    }
                      ...
                      <div key={index} className={dayClasses} onClick={() => openModalForDate(formatDateKey(day.date))}>
                        ...
                        {day.isVacationDay ? (
                          <div className="flex items-center justify-center h-full">
                            <span className="text-[8px] md:text-xs font-bold text-white uppercase tracking-wider">FERIE</span>
                          </div>
                        ) : (
                          <>
                            <div className="space-y-0.5">
                              {day.events.slice(0, window.innerWidth < 768 ? 1 : 2).map((event) => (
                                <div
                                  key={event.id}
                                  className={`text-[8px] md:text-[10px] leading-tight px-0.5 md:px-1 py-0.5 rounded text-white truncate font-medium ${
                              ...
                              {day.events.length > (window.innerWidth < 768 ? 1 : 2) && <div className="text-[8px] md:text-[10px] text-gray-300 px-0.5 md:px-1 font-medium">+{day.events.length - (window.innerWidth < 768 ? 1 : 2)}</div>}
                            </div>
                            <div className="absolute top-0.5 md:top-1 right-0.5 md:right-1">
                              {day.isAvailable ? <CheckCircle className="h-2.5 w-2.5 md:h-3.5 md:w-3.5 text-green-400" /> : <AlertTriangle className="h-2.5 w-2.5 md:h-3.5 md:w-3.5 text-red-400" />}
                            </div>
                          </>
                        )}
                      </div>
